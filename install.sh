...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
timedatectl set-timezone "$(curl -s https://ipapi.co/timezone)" ; timedatectl set-ntp true 
reflector -p https -c "$(curl -s https://ipapi.co/country_name)" -f 2 --save /etc/pacman.d/mirrorlist 2>/dev/null
gpg --list-keys ; echo "keyserver-options auto-key-retrieve" >> .gnupg/gpg.conf ;echo "keyserver hkp://pgp.mit.edu" >> .gnupg/gpg.conf 
...
genfstab -U /mnt >> /mnt/etc/fstab ; arch-chroot /mnt ln -sf "$(/usr/share/zoneinfo/"$(curl -s https://ipapi.co/timezone)")" /etc/localtime 
hwclock --systohc ; locale-gen --purge en_US.UTF-8 ; echo -e 'LANG="en_US.UTF-8"\nLANGUAGE="en_US:en"\n'  >> /etc/locale.conf 
